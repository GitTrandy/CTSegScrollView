...
- (void)createView
{
    ...
    for (NSInteger i = 0 ; i < [self.titleArray count]; i++) {
        ...
        if (self.delegate) {
            ...
            view.frame = CGRectMake(self.frame.size.width * i, 30, view.frame.size.width,view.frame.size.height);
        }else
        {
            view = [[UIView alloc] initWithFrame:CGRectMake(self.frame.size.width*i, 30, self.frame.size.width, self.frame.size.height - 30)];
        }
